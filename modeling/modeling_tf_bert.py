    ...
    def saliency_step(self, inputs, training=False, saliency_agg="L2", norm=None):
        ...
        with tf.GradientTape() as tape:
            outputs = self.call(inputs, training=training,
                                output_hidden_states=True, output_explanations=True)
        ...
        # Respect to label
        saliency = tf.gather(saliency, labels, axis=1, batch_dims=1)
        
        # Max/Min Normalize
        saliency = saliency * fp32_effective_mask
        if norm == "z-norm":
            avgs = tf.reduce_sum(saliency, axis=-1) / (tf.reduce_sum(fp32_effective_mask, axis=-1) + 1e-15)
            stds = tf.sqrt(tf.reduce_sum(((saliency - tf.expand_dims(avgs, axis=-1)) ** 2) * fp32_effective_mask, axis=-1) / (tf.reduce_sum(fp32_effective_mask, axis=-1) + 1e-15))
            saliency = (saliency - tf.expand_dims(avgs, axis=-1)) / (tf.expand_dims(stds, axis=-1) + 1e-15)
        elif norm == "min-max":
            saliency = (saliency - tf.expand_dims(tf.reduce_min(saliency, axis=-1), axis=-1)) / (tf.expand_dims(tf.reduce_max(saliency, axis=-1), axis=-1) - tf.expand_dims(tf.reduce_min(saliency, axis=-1), axis=-1) + 1e-15)
        
        return {"saliencies": saliency}
